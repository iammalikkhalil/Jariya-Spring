...
class UserRepositoryImpl(
    ...
    override fun isAlreadyParent(child: String, parent: String): Boolean {
        ...
        while (true) {
            val parentUser =
                userJpaRepository.findById(parentId.toUUID()).orElse(null) ?: break
    ...
    @Transactional(readOnly = true)
    override fun countAllActiveVerifiedUsers(): Long =
        userJpaRepository.countAllActiveVerifiedUsers()
    ...
    override fun registerUser(user: UserModel): Boolean {
        return try {
            ...
            // ✅ EVENT
            eventPublisher.publishEvent(UserStatsChangedEvent())
    ...
    override fun updateUser(user: UserModel): Boolean {
        return try {
            ...
            // ❌ No event — does not affect stats
            true
        } catch (e: Exception) {
    ...
    override fun verifyUser(email: String): Boolean {
        return try {
            val updated = userJpaRepository.verifyUser(email) > 0
            if (updated) {
                // ✅ EVENT
                eventPublisher.publishEvent(UserStatsChangedEvent())
            }
    ...
    override fun markUserAsVerified(userId: String): Boolean {
        return try {
            ...
            val updated =
                user.copy(isVerified = true, updatedAt = Instant.now())
